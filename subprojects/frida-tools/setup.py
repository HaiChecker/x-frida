...
def main():
    setup(
        ...
        description="Frida CLI tools (stealth build)",
        long_description="CLI tools for [x-frida](https://github.com/HaiChecker/x-frida).",
        long_description_content_type="text/markdown",
        author="Frida Developers",
        author_email="oleavr@frida.re",
        url="https://github.com/HaiChecker/x-frida",
        install_requires=[
            "colorama >= 0.2.7, < 1.0.0",
            "x-frida >= 17.9.0, < 18.0.0",
        ...
        entry_points={
            "console_scripts": [
                "x-frida = frida_tools.repl:main",
                "x-frida-ls-devices = frida_tools.lsd:main",
                "x-frida-ps = frida_tools.ps:main",
                "x-frida-kill = frida_tools.kill:main",
                "x-frida-ls = frida_tools.ls:main",
                "x-frida-rm = frida_tools.rm:main",
                "x-frida-pull = frida_tools.pull:main",
                "x-frida-push = frida_tools.push:main",
                "x-frida-discover = frida_tools.discoverer:main",
                "x-frida-trace = frida_tools.tracer:main",
                "x-frida-strace = frida_tools.stracer:main",
                "x-frida-itrace = frida_tools.itracer:main",
                "x-frida-join = frida_tools.join:main",
                "x-frida-create = frida_tools.creator:main",
                "x-frida-compile = frida_tools.compiler:main",
                "x-frida-pm = frida_tools.pm:main",
                "x-frida-apk = frida_tools.apk:main",
            ]
        },
    )
